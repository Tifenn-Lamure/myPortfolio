  ...
  return (
    <div className={`relative ${IsAtLeft ? 'mr-4 sm:mr-8 ' : 'ml-4 sm:ml-8 '}`}>
      <div className={`h-[12rem] sm:h-[24rem] bg-light-violet p-1 sm:p-4 border-black border-y-4 relative z-10 flex gap-2 sm:gap-10 items-center ${IsAtLeft ? 'border-r-4 rounded-r-lg' : 'border-l-4 rounded-l-lg flex-row-reverse'}`}>
          ...
          {!isViewMore ?
            <>
              <div className="text-[12px] sm:text-[18px] md:text-[24px] text-center text-white leading-[1rem] sm:leading-normal">{TextProject}</div>
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              {SkillTab.map((SkillTab, index) => (
                <SkillBtn BtnWidth="w-[3.5rem] sm:w-32" key={index} BtnHeight="h-[2rem]" SkillBtnText={SkillTab} textSize="text-[10px] sm:text-[18px]" BtnColorBack="bg-very-light-violet" BtnColorFront="bg-dark-violet" className="w-48"/>
              ))}
              </div>
              <div className="flex flex-col items-center justify-center mt-[1rem]">
                <a href={LinkProject} target="_blank" without="true" rel="noopener noreferrer" className="text-center text-[12px] sm:text-[18px] md:text-[24px] text-white pb-[.5rem] hover:underline">Click for my repositories</a>
