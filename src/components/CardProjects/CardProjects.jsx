    ...
    <div className={`relative ${IsAtLeft ? 'mr-8 ' : 'ml-8 '}`}>
      <div className={`h-[24rem] bg-light-violet p-4 border-black border-y-4 relative z-10 flex gap-10 items-center ${IsAtLeft ? 'border-r-4 rounded-r-lg' : 'border-l-4 rounded-l-lg flex-row-reverse'}`}>
          ...
          <div className="flex flex-col gap-[1rem] items-center w-[50%]">
            <div className="text-[24px] text-center text-white">{TextProject}</div>
            {!isViewMore ?
            ...
            <div className="flex flex-col items-center justify-center">
              ...
              {SkillTab.map((SkillTab) => (
                <SkillBtn BtnWidth="w-40" key={SkillTab.id} BtnHeight="h-[2rem]" SkillBtnText={SkillTab} textSize="text-[18px]" BtnColorBack="bg-very-light-violet" BtnColorFront="bg-dark-violet" className="w-48"/>
              ))}
              </div>
              <div className="flex flex-col items-center justify-center mt-[2rem]">
              <a href={LinkProject} target="_blank" without rel="noopener noreferrer" className="text-[24px] text-white pb-[1.5rem] hover:underline">Click for my repositories</a>
                <div className="h-[48px]">
                  <button className="text-center text-2xl text-center w-48 bg-very-light-violet rounded-lg border-black border-4 relative z-10 hover:text-lg hover:h-8 hover:w-44 hover:leading-6" onClick={() => setViewMore(false)}>View Less</button>
                </div>
              </div>
            </div>
            }
